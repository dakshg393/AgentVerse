...
export default function Dashboard() {
  ...
  return (
    <section
      // ref={sectionRef} // Uncomment if using Vanta background
      className="flex items-center pb-16 justify-center flex-col min-h-screen  bg-no-repeat bg-cover bg-center"
    >
      <span className="mt-10">
        <h1 className="text-6xl font-extrabold font-montserrat text-transparent text-center leading-tight bg-gradient-to-r dark:from-white from-black from-20% to-theme-color to-[80%] bg-clip-text">
          Meet Your AI Experts
        </h1>
        <p className="mt-4 text-2xl font-medium text-center">
          Always here to help – smart, fast & personalized
      ...
      <div className="p-8 flex items-center justify-center  gap-8 flex-wrap ">
        {aiServices.map((service, index) => (
          <Card
            key={index}
            className="cursor-pointer hover:shadow-lg transition-transform duration-300 hover:-translate-y-2 hover:border-theme-color border-white backdrop-blur-xs bg-transparent w-[80%] h-auto md:w-80 md:h-50 text-center"
            onClick={() => router.push(service.href)}
          >
            <CardHeader className="flex flex-col items-center">
              <span className="p-4 rounded-full" style={{ backgroundImage: service.color }}>
